...
class Archive(object):
    ...
    def __init__(self, config=None):
        ...
        if approachActivation()[2]==True:
            self.archive = {'hypervolume': [], 'first_risk': [], 'counter': [], 'threshold': [], 'cumWaitTime': [],
               'cumCrashes': [], 'avgFitness': [], 'time_nd': [], 'risk_nd': [], 'case': [], 'id': [], 'information_type': [], 'iteration': []}
        else:
            self.archive = {'hypervolume': [], 'first_risk': [], 'counter': [], 'threshold': [], 'cumWaitTime': [],
    ...
    def update_archive(self, agent, risk_tol, threshold_tol, hv_tol, archive_index, hypervolume, non_dominated_risk, case):
        ...
        if archive_index != -1 and agent.archive_updated == False:
            ...
            if approachActivation()[2]==True:
                self.archive['id'][archive_index] = agent.id
                self.archive['iteration'][archive_index] = len(
                    [value for value in list(zip(self.archive['id'], self.archive['information_type'])) if
                    value[0] == agent.id and value[1] == 'self'])
        ...
        # Else if we selected a new state and tried it and archive not yet updated
            ...
            # Case new Threshold for an existing state
                ...
                if approachActivation()[2]==True:
                    self.archive['information_type'].append('self')
                    iteration_id = len(
                        [value for value in list(zip(self.archive['id'], self.archive['information_type'])) if
                         value[0] == agent.id and value[1] == 'self'])
                    self.archive['iteration'].append(iteration_id)
    ...
    def set_archive1(self, risk_tol, threshold_tol, hv_tol, sigma, testing=False):
        self.archive = pickle.load(open(
            var.path + '/Archives/{0}_{1}_{2}_{3}{4}.pkl'.format(risk_tol, threshold_tol, hv_tol, sigma,
                                                                  '_cd' if testing else ''), 'rb'))

    def add_to_archive(self, entry: dict):
        for key in entry.keys():
            lazy_code = f"self.archive['{key}'].append(entry['{key}'])"  # changing archive to self.archive
            eval(lazy_code)
    ...
    def best_worst_approach(self):
        dictionary = self.archive
        return dictionary
